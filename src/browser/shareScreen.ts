import type { DataChannelMessageBody } from './chat';
...
async function replaceVideoTrack(track: MediaStreamTrack): Promise<boolean> {
  ...
  const videoSenders = senders.filter((s) => s.track?.kind === 'video' && s.transport !== null);
...
export async function shareScreen() {
  ...
  if (dc?.readyState === 'open') {
    const res = { type: 'screen-share', active: true } as DataChannelMessageBody;
...
async function stopSharing() {
  ...
  if (dc?.readyState === 'open') {
    const res = { type: 'screen-share', active: false } as DataChannelMessageBody;
    dc.send(JSON.stringify(res));
  }
