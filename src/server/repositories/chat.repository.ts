...
import type { Conversation, Conversations, FetchConversationsResponse, MessageDTO } from '../../shared/types';
import { getUserPair } from '../units/userPair';
import {
  prepareCreateConvQuery,
  prepareFetchAllConversations,
...
export const getOrCreateConversation = (user1_id: number, user2_id: number): number => {
  ...
  const conversation = database.prepare(prepareCreateConvQuery).get(u1, u2) as Conversation;

  return conversation.id;
};

export const fetchConversations = (query: FetchConversationSchema): FetchConversationsResponse[] | undefined => {
  const rows = database
    .prepare(prepareFetchConvQuery)
    .all(query.conversation_id, query.limit ?? 5) as FetchConversationsResponse[];
...
export const sendConversationMessages = (sender_id: number, query: SendConversationMessageSchema): number => {
  const result = database
    .prepare(prepareSendMessageQuery)
    .run(query.conversation_id, sender_id, query.type, query.content);
  return result.lastInsertRowid as number;
};

export const fetchAllConversations = (sender_id: number): Conversations[] | undefined => {
  return database.prepare(prepareFetchAllConversations).all(sender_id, sender_id, sender_id) as
    | Conversations[]
    | undefined;
};
